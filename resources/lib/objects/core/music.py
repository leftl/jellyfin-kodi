...
class Music(KodiDb):
    ...
    def __init__(self, server, jellyfindb, musicdb, direct_path, *args, **kwargs):
    ...
    def get_song_path_filename(self, obj, api):
        ...
        if self.direct_path:
            ...
            obj['Path'] = "http://127.0.0.1:57578/Audio/%s/" % obj['Id'] # use redirect to append api_key
    ...
    def remove(self, item_id, e_item, *args, **kwargs):
        ...
        if obj['Media'] == 'song':
            ...
            for item in self.jellyfin_get_item_by_wild_id(*values(obj, QUEM.get_item_by_wild_obj)):
                if item[1] == 'album':
