...
class API(object):
    ...
    def get_backdrops(self, item_id, tags, query=None):
        ...
        for index, tag in enumerate(tags):
            ...
            artwork = "%s/Items/%s/Images/Backdrop/%s?%s" % (self.server, item_id, index, urllib.urlencode(query))
    ...
    def get_artwork(self, item_id, image, tag=None, query=None):
        ...
        artwork = "%s/Items/%s/Images/%s/0?%s" % (self.server, item_id, image, urllib.urlencode(query))
