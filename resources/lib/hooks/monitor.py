...
class Monitor(xbmc.Monitor):
    ...
    def _get_server(self, method, data):
        ...
        try:
            ...
            server = Jellyfin(data['ServerId']).get_client()
        except Exception:
            server = Jellyfin().get_client()
    ...
    def get_plugin_video_jellyfin_method(self):
    ...
    def onNotification(self, sender, method, data):
        ...
        if sender == 'plugin.video.jellyfin':
            ...
            if method not in self.get_plugin_video_jellyfin_method():
                return
    ...
    def on_notification(self, sender, method, data):
        ...
        if method == 'Playlist.OnClear' and data['playlistid'] == self.playlistid: # set in webservice.py
            window('jellyfin.play.widget', clear=True)
    ...
    def post_capabilities(self, server):
        ...
        server['api'].post_capabilities({
            ...
            'IconUrl': "https://raw.githubusercontent.com/MediaBrowser/plugin.video.jellyfin/master/kodi_icon.png",
        })
    ...
    def LoadServer(self, server, data, *args, **kwargs):
        ...
        if not data['ServerId']:
            window('jellyfin.server.state.json', server.get_state())
        else:
            window('jellyfin.server.%s.state.json' % data['ServerId'], server.get_state())
            current = window('jellyfin.server.states.json') or []
            current.append(data['ServerId'])
            window('jellyfin.server.states.json', current)

    def StopServer(self, server, data, *args, **kwargs):

        if not data['ServerId']:
            window('jellyfin.server.state', clear=True)
        else:
            window('jellyfin.server.%s.state' % data['ServerId'], clear=True)
            current = window('jellyfin.server.states.json')
    ...
    def VideoLibrary_OnUpdate(self, server, data, *args, **kwargs):
        ...
        try:
            ...
            if 'id' in data and 'type' in data and window('jellyfin.context.resetresume.bool'):
    ...
    def Playlist_OnAdd(self, server, data, *args, **kwargs):
        ...
        if data['position'] == 0:
            ...
            if self.playlistid == data['playlistid'] and data['item']['type'] != 'unknown':
                ...
                window('jellyfin.autoplay', clear=True)

            """
            if data['playlistid'] == 0:
                window('jellyfin.playlist.audio.bool', True)
            else:
                window('jellyfin.playlist.audio', clear=True)
            """

            self.playlistid = data['playlistid']

        LOG.info(data)
        if window('jellyfin.playlist.start') and data['position'] == int(window('jellyfin.playlist.start')) + 1:

            LOG.info("--[ playlist ready ]")
            window('jellyfin.playlist.ready.bool', True)
            window('jellyfin.playlist.start', clear=True)
