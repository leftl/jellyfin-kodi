...
class RequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def strm(self):
        ...
        if settings('pluginSimple.bool'):
            path = "plugin://plugin.video.jellyfin/?mode=playsingle&id=%s" % params['Id']
        ...
        path = "plugin://plugin.video.jellyfin/?mode=play&id=%s" % params['Id']
        if params.get('server'):
            path += "&server=%s" % params['server']
    ...
    def images(self):
        ...
        '''
        image = xbmc.translatePath("special://home/addons/plugin.video.jellyfin/resources/icon.png").decode('utf-8')
...
class Redirect(threading.Thread):
    ...
    def __init__(self, redirect, server_id=None):
        ...
        self.server = Jellyfin(server_id).get_client()
        threading.Thread.__init__(self)
    ...
    def run(self):
        ...
        """
        LOG.info("[DEBUG-TRANSITION] path: %s", self.path)
...
class PlayWidget(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            if not server and not window('jellyfin_online.bool'):
                dialog("notification", heading="{jellyfin}", message=_(33146), icon=xbmcgui.NOTIFICATION_ERROR)
            ...
            objects.utils.disable_busy_dialog()
            window('jellyfin.play.widget.bool', True)
            play.start_playback()
...
class PlayFolder(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            if not window('jellyfin.playlist.plugin.bool'): # default.py wait for initial item to start up
                try:
                    try:
