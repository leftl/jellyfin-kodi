...
class Library(threading.Thread):
    ...
    def service(self):
        ...
        for threads in (self.jellyfin_threads, self.writer_threads['updated'], self.writer_threads['userdata'],
        ...
        if (self.pending_refresh and not self.download_threads and not self.writer_threads['updated'] and
            ...
            if not xbmc.getCondVisibility('Window.IsMedia'):
                ...
                window('jellyfin.updatewidgets.bool', True)

        elif window('jellyfin.updatewidgets.bool') and not xbmc.getCondVisibility('Window.IsMedia'):

            ''' In case an update happened but we were not on the homescreen and 
                now we are, force widget to update.
            '''
            window('jellyfin.updatewidgets', clear=True)
            xbmc.executebuiltin('UpdateLibrary(video)')
    ...
    def worker_sort(self):
        ...
        if self.removed_queue.qsize() and len(self.jellyfin_threads) < 2:
            ...
            self.jellyfin_threads.append(new_thread)
    ...
    def fast_sync(self, plugin):
        ...
        with Database('jellyfin') as jellyfindb:
            boxsets = dict(jellyfin_db.JellyfinDatabase(jellyfindb.cursor).get_items_by_media('set') or {})
            ...
            with self.database as kodidb:
                with Database('jellyfin') as jellyfindb:
                    ...
                    while True:
                        ...
                        obj = MEDIA[item['Type']](self.library.server, jellyfindb, kodidb, self.library.direct_path)[item['Type']]
            ...
            with self.database as kodidb:
                with Database('jellyfin') as jellyfindb:
                    ...
                    while True:
                        ...
                        obj = MEDIA[item['Type']](self.library.server, jellyfindb, kodidb, self.library.direct_path)['UserData']
                        self.library.update_progress_dialog(api.API(item).get_naming())
            ...
            with self.database as kodidb:
                with Database('jellyfin') as jellyfindb:
                    ...
                    while True:
                        ...
                        obj = MEDIA[item['Type']](self.library.server, jellyfindb, kodidb, self.library.direct_path)['Remove']
