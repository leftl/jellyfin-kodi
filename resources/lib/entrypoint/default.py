...
class Events(object):
    ...
    def __init__(self):
        ...
        if 'content_type' in params:
            ...
            window('jellyfin.plugin.content.type', params['content_type'])
            CONTENT_TYPE = params['content_type']
        else:
            CONTENT_TYPE = window('jellyfin.plugin.content.type') or None
        ...
        if '/extrafanart' in base_url:
            ...
            while not window('jellyfin.playlist.play.bool'):
                xbmc.sleep(50)

                if window('jellyfin.playlist.aborted.bool'):
            ...
            window('jellyfin.playlist.aborted', clear=True)
            window('jellyfin.sync.pause', clear=True)
            window('jellyfin.playlist.plugin', clear=True)
            ...
            xbmc.executebuiltin('ShowPicture(%s/Items/%s/Images/Primary)' % (Jellyfin(server)['auth/server-address'], params['id']))
        elif mode == 'deviceid':
...
def get_server(server=None):
    ...
    try:
        JELLYFIN = Jellyfin(server).get_client()
        ...
        for i in range(300):
            ...
            if server is None and window('jellyfin_online.bool'):
                Jellyfin().set_state(window('jellyfin.server.state.json'))

                break

            if server is not None and server in window('jellyfin.server.states.json') or []:
...
def listing():
    ...
    for server in servers:
        ...
        if server.get('ManualAddress'):
            context.append((_(30500), "RunPlugin(plugin://plugin.video.jellyfin/?mode=setssl&server=%s)" % server['Id']))
    ...
    directory(_(33059), "plugin://plugin.video.jellyfin/?mode=texturecache", False)
    ...
    directory("Changelog", "plugin://plugin.video.jellyfin/?mode=changelog", False)
...
def manage_libraries():
    ...
    if kodi_version() >= 18:
        directory(_(33202), "plugin://plugin.video.jellyfin/?mode=patchmusic", False)
...
def changelog():
    ...
    resp = dialog("select", heading="{jellyfin}", list=[version, _(33212)])

    if resp < 0:
        return

    if resp:
        src = "https://api.github.com/repos/MediaBrowser/plugin.video.jellyfin/releases/latest"
    else:
        src = "https://api.github.com/repos/MediaBrowser/plugin.video.jellyfin/releases/tags/%s" % version
    ...
    try:
        ...
        dialog("textviewer", heading="{jellyfin}", text=response['body'])
    except Exception as error:

        LOG.error(error)
        dialog("notification", heading="{jellyfin}", message=_(33204), icon="{jellyfin}", time=1000, sound=False)
...
def browse(media, view_id=None, folder=None, server_id=None):
    ...
    if view_id:
        ...
        view = JELLYFIN['api'].get_item(view_id)
    ...
    if folder == 'recentlyadded':
        listing = JELLYFIN['api'].get_recently_added(None, view_id, None)
    elif folder == 'genres':
        listing = JELLYFIN['api'].get_genres(view_id)
    elif media == 'livetv':
        listing = JELLYFIN['api'].get_channels()
        ...
        listing = JELLYFIN['api'].get_seasons(folder)
    elif media != 'files':
        listing = downloader.get_filtered_section(folder or view_id, get_media_type(content_type), None, False, None, None, None, None, server_id)
    else:
    ...
    if listing:
        ...
        listitems = objects.ListItem(JELLYFIN['auth/server-address'])
        list_li = []
        ...
        for item in listing:
            ...
            if item.get('IsFolder'):
                ...
                if item['Type'] == 'Photo' and CONTENT_TYPE == 'video':
                    ...
                    path = "plugin://plugin.video.jellyfin/?mode=photoviewer&id=%s" % item['Id']
...
def get_fanart(item_id, path, server_id=None):
    ...
    if not xbmcvfs.exists(directory):
        ...
        item = JELLYFIN['api'].get_item(item_id)
        obj = objects.map(item, 'Artwork')
        backdrops = api.API(item, JELLYFIN['auth/server-address']).get_all_artwork(obj)
...
def add_user(permanent=False):
    ...
    get_server()
    session = JELLYFIN['api'].get_device(JELLYFIN['config/app.device_id'])
    hidden = None if settings('addUsersHidden.bool') else False
    users = JELLYFIN['api'].get_users(False, hidden)
    ...
    while True:
        ...
        if not result: # Add user
            ...
            dialog("notification", heading="{jellyfin}", message="%s %s" % (_(33067), user['Name']), icon="{jellyfin}", time=1000, sound=False)
        else: # Remove user
            ...
            dialog("notification", heading="{jellyfin}", message="%s %s" % (_(33066), user['UserName']), icon="{jellyfin}", time=1000, sound=False)
...
def get_themes():
    ...
    get_server()
    items = {}
    server = JELLYFIN['auth/server-address']
    token = JELLYFIN['auth/token']
