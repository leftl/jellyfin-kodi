...
class Sync(object):
    ...
    def mapping(self, forced=False):
        ...
        if self.sync['Libraries']:
            if not forced and not dialog("yesno", heading="{jellyfin}", line1=_(33102)):
        ...
        with self.library.database_lock:
            with Database() as videodb:
                with Database('jellyfin') as jellyfindb:
                    ...
                    obj = Movies(self.server, jellyfindb, videodb, self.direct_path, True)
    ...
    def movies_compare(self, library, obj, jellyfindb):
    ...
    def patch_music(self, notification=False):
        ...
        if notification:
            dialog("notification", heading="{jellyfin}", message=_('task_success'), icon="{jellyfin}", time=1000, sound=False)
