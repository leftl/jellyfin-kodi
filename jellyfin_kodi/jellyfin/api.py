...
def jellyfin_url(client, handler):
    return  "%s/%s" % (client['config/auth.server'], handler)
...
class API(object):
    ...
    def get_sync_queue(self, date, filters=None):
        return  self._get("Jellyfin.Kodi.SyncQueue/{UserId}/GetItems", params={
                    'LastUpdateDT': date,
                    'filter': filters or None
                })

    def get_server_time(self):
        return  self._get("Jellyfin.Kodi.SyncQueue/GetServerDateTime")
    ...
    def is_valid_episode(self, parent_id, name, item_id):
        ...
        ''' Special function to detect if episodes are displayed in jellyfin.
