...
def should_stop():
    ...
    '''
    if xbmc.Monitor().waitForAbort(0.00001) or window('jellyfin_should_stop.bool') or not window('jellyfin_online.bool'):
        LOG.info("exiiiiitttinggg")

        raise Exception('StopCalled')

    if window('jellyfin.sync.pause.bool'):
...
def validate(path):
    ...
    if not xbmcvfs.exists(path):
        ...
        if dialog("yesno", heading="{jellyfin}", line1=string):
