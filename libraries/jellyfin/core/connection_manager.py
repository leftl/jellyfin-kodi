...
class ConnectionManager(object):
    ...
    def get_connect_url(self, handler):
       return "https://connect.jellyfin.media/service/%s" % handler
    ...
    def _add_authentication_info_from_connect(self, server, connection_mode, credentials, options={}):
        ...
        try:
            auth = self._request_url({
                'url': self.get_jellyfin_url(get_server_address(server, connection_mode), "Connect/Exchange"),
                'type': "GET",
                'dataType': "json",
                'verify': options.get('ssl'),
                'params': {
